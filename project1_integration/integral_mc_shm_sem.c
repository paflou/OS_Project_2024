...
int main(int argc, char *argv[]) {
  ...
  long number_of_processors = sysconf(_SC_NPROCESSORS_CONF); //Get the number of processors in the system for error detection
  ...
  if (argc == 2) { //Number of processes to create if not default (4)
    ...
    if(Processes > number_of_processors || Processes<=0){
      printf("Wrong input!\n");
      exit(1);
    } 
  }
